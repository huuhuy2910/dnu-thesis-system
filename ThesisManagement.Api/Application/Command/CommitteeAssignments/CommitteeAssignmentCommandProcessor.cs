...
{
    public interface ICommitteeAssignmentCommandProcessor
    {
    ...
    public class CommitteeAssignmentCommandProcessor : ICommitteeAssignmentCommandProcessor
    {
        private readonly ApplicationDbContext _db;
        private readonly IUnitOfWork _uow;
        private readonly ILogger<CommitteeAssignmentCommandProcessor> _logger;

        public CommitteeAssignmentCommandProcessor(ApplicationDbContext db, IUnitOfWork uow, ILogger<CommitteeAssignmentCommandProcessor> logger)
        {
            _db = db;
            _uow = uow;
            _logger = logger;
        ...
        // Tags
        public async Task<ApiResponse<List<TagReadDto>>> GetTagsAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                var tags = await _db.Tags.AsNoTracking()
                ...
                var res = ApiResponse<List<TagReadDto>>.SuccessResponse(tags);
                res.HttpStatusCode = StatusCodes.Status200OK;
                return res;
            }
            catch (Exception ex)
            {
                ...
                return ApiResponse<List<TagReadDto>>.Fail("Không thể lấy danh sách tag", StatusCodes.Status500InternalServerError);
            }
        }
