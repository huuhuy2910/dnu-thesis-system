    ...
    public class RollbackStudentWorkflowTestDataCommandProcessor : IRollbackStudentWorkflowTestDataCommandProcessor
    {
        ...
        public async Task<OperationResult<TopicWorkflowRollbackResultDto>> ExecuteAsync(string? topicCode = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(topicCode))
            {
                ...
                topics = topics
                    .Where(x => string.Equals(x.TopicCode, normalizedTopicCode, StringComparison.OrdinalIgnoreCase))
                    .ToList();

                if (topics.Count == 0)
                    return OperationResult<TopicWorkflowRollbackResultDto>.Failed("Topic not found for current user", 404);
            }
            ...
            try
            {
                ...
                if (progressMilestones.Count > 0)
                    _db.ProgressMilestones.RemoveRange(progressMilestones);
                if (topicLecturers.Count > 0)
                    _db.TopicLecturers.RemoveRange(topicLecturers);
                if (topicTags.Count > 0)
                    _db.TopicTags.RemoveRange(topicTags);
                if (conversationMembers.Count > 0)
                    _db.ConversationMembers.RemoveRange(conversationMembers);
                if (directConversations.Count > 0)
                    _db.Conversations.RemoveRange(directConversations);
                if (topics.Count > 0)
            ...
            catch (Exception ex)
            {
                ...
                return OperationResult<TopicWorkflowRollbackResultDto>.Failed($"Rollback failed: {ex.Message}", 500);
            }
        }
