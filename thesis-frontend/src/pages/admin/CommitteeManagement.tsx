...
import {
  committeeService,
  type EligibleTopicSummary,
} from "../../services/committee-management.service";
import type {
...
const API_BASE_URL = (
  (import.meta.env.VITE_API_BASE_URL as string) || "http://localhost:5180"
)
  .trim()
  .replace(/\/+$/, "");
...
const ROLE_CONFIG: RoleConfig[] = [
  ...
  {
    id: "reviewer1",
    label: "Phản biện (Ủy viên) 1",
    apiRole: "Phản biện (Ủy viên)",
  },
  { id: "secretary", label: "Thư ký", apiRole: "Thư ký" },
  {
    id: "reviewer2",
    label: "Phản biện (Ủy viên) 2",
    apiRole: "Phản biện (Ủy viên)",
  },
  {
    id: "reviewer3",
    label: "Phản biện (Ủy viên) 3",
    apiRole: "Phản biện (Ủy viên)",
  },
];
...
function normalizeTopicItem(item: RawTopicItem): TopicTableItem | null {
  if (!item) return null;
  const topicCode =
    item.topicCode ?? item.topic_code ?? item.code ?? item.topicId;
  ...
  return {
    ...
    title: String(title),
    studentName:
      item.studentName ?? item.studentFullName ?? item.student ?? null,
    supervisorName:
      item.supervisorName ?? item.lecturerName ?? item.supervisor ?? null,
    supervisorCode: item.supervisorCode ?? item.lecturerCode ?? null,
    supervisorLecturerProfileID:
      item.supervisorLecturerProfileID ??
      item.supervisorLecturerProfileId ??
      item.supervisorLecturerProfileID ??
      null,
    supervisorLecturerCode:
      item.supervisorLecturerCode ??
      item.supervisorLecturerCode ??
      item.supervisorLecturerCode ??
      item.supervisorCode ??
      item.supervisorLecturerCode ??
    ...
    ...(item.supervisorUserCode
      ? { supervisorUserCode: String(item.supervisorUserCode) }
      : {}),
    specialty:
      item.specialty ?? item.specialtyName ?? item.specialtyCode ?? null,
...
function normalizeLecturerItem(item: RawLecturerItem): LecturerOption | null {
  if (!item) return null;
  const lecturerProfileId =
    item.lecturerProfileId ?? item.lecturerProfileID ?? item.id;
...
function computeNextSessionCandidate(
  items: CommitteeAssignmentListItem[]
): StatsSnapshot["nextSession"] {
        ...
        return (
          ...
          >
            ...
            <div className="min-w-0">
              <p
                className={`text-sm font-semibold ${
                  active ? "text-[#1F3C88]" : "text-[#1D2753]"
                }`}
              >
            ...
            {index < WIZARD_STEPS.length - 1 && (
              <span
                className="hidden flex-1 border-t border-dashed border-[#D9E1F2] sm:block"
                aria-hidden
              />
            )}
          </li>
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: accent ? ACCENT_COLOR : MUTED_BORDER,
      }}
    >
      <div
        className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${
          accent ? "bg-[#00B4D8]/10" : "bg-[#1F3C88]/10"
        }`}
      >
        <div className={`${accent ? "text-[#00B4D8]" : "text-[#1F3C88]"}`}>
          {icon}
        </div>
      </div>
      <div>
        ...
        <p
          className={`mt-1 text-3xl font-bold ${
            accent ? "text-[#00B4D8]" : "text-[#1F3C88]"
          }`}
        >
          {value}
        </p>
    ...
    <div className="flex h-full flex-col justify-between rounded-3xl border border-[#D9E1F2] bg-gradient-to-br from-[#1F3C88] to-[#162B61] p-6 text-white shadow-lg">
      ...
      <div className="mt-4 flex flex-col gap-3 text-sm text-white/90">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>
            Giờ bắt đầu:{" "}
            {info?.startTime ? formatTime(info.startTime) : "Chờ xác định"}
          </span>
        </div>
        <div className="flex items-center gap-2">
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#E5ECFB] bg-[#F8FAFF]">
                  <th className="px-4 py-3 text-left font-semibold text-[#1F3C88]">
                    Mã hội đồng
                  </th>
                  <th className="px-4 py-3 text-left font-semibold text-[#1F3C88]">
                    Phòng
                  ...
                  <th className="px-4 py-3 text-center font-semibold text-[#1F3C88]">
                    Hành động
                  </th>
                </tr>
              </thead>
              <tbody>
                {data.length === 0 ? (
                  <tr>
                    <td
                      colSpan={7}
                      className="px-4 py-6 text-center text-[#4A5775]"
                    >
                      Chưa có dữ liệu
                    </td>
                  </tr>
                  ...
                  data.map((item) => (
                    <tr
                      ...
                      className="border-b border-[#E5ECFB] hover:bg-[#F8FAFF]"
                    >
                      <td className="px-4 py-3 font-semibold text-[#1F3C88]">
                        {item.committeeCode}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {item.tagCodes?.slice(0, 2).map((tag: string) => (
                            <span
                              key={tag}
                              className="rounded-full bg-[#1F3C88]/10 px-2 py-1 text-xs text-[#1F3C88]"
                            >
                              {resolveTagLabel(tag)}
                            </span>
                          ))}
          ...
          <div className="mt-4 flex items-center justify-between">
            <span className="text-xs text-[#4A5775]">
              Trang {page} của {Math.max(1, Math.ceil(total / pageSize))} (
              {total} hội đồng)
            </span>
            ...
            <>
              <div className="max-h-[400px] space-y-3 overflow-y-auto">
                {topics.map((topic) => (
                  <div
                    key={topic.topicCode}
                    className="rounded-2xl border border-[#E5ECFB] bg-[#F8FAFF] p-4"
                  >
                    <div className="flex items-start gap-3">
                      {mode === "multi-select" && onToggleTopic && (
                        <input
                          ...
                          onChange={(e) =>
                            onToggleTopic(topic.topicCode, e.target.checked)
                          }
                          className="mt-1"
                        />
                      )}
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-[#4A5775]">
                          Mã: {topic.topicCode}
                        </p>
                        {topic.studentName && (
                          <p className="text-sm text-[#4A5775]">
                            Sinh viên: {topic.studentName}
                          </p>
                        )}
                        <div className="mt-2 flex flex-wrap gap-1">
                          {topic.tagDescriptions?.map((tag) => (
                            <span
                              key={tag}
                              className="rounded-full bg-[#1F3C88]/10 px-2 py-0.5 text-xs text-[#1F3C88]"
                            >
                              {tag}
                            </span>
                          ))}
...
function AssignTopicModal({
  ...
  return (
    <ModalShell
      title="Gán đề tài cho hội đồng"
      subtitle={topic.title}
      onClose={onClose}
    >
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-semibold text-[#1F3C88]">
            Hội đồng
          </label>
          <select
            value={committeeCode}
        ...
        <div className="space-y-3">
          {result.map((item) => (
            ...
            >
              ...
              <p className="text-sm text-[#4A5775]">
                Đề tài gán: {item.assignedCount ?? 0}
              </p>
            </div>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <p className="text-[#4A5775]">
            Hệ thống sẽ tự động phân công các đề tài đủ điều kiện cho các hội
            đồng phù hợp.
          </p>
          <div className="flex gap-2 pt-4">
            <button
...
const CommitteeManagement: React.FC = () => {
  ...
  const [persistedCommitteeCode, setPersistedCommitteeCode] = useState<
    string | null
  >(null);
  const [wizardInit, setWizardInit] = useState<CommitteeCreateInitDto | null>(
    null
  );
  ...
  const [assignedTopics, setAssignedTopics] = useState<
    Record<SessionId, AssignedTopicSlot[]>
  >({
    1: [],
    2: [],
  });
  ...
  const defaultModalState = useMemo(
    () => ({ eligibleTopics: false, assignTopic: false, autoAssign: false }),
    []
  );
  const [modals, setModals] = useState(defaultModalState);
  const [assigningTopic, setAssigningTopic] =
    useState<EligibleTopicSummary | null>(null);
  const [autoAssignResult, setAutoAssignResult] = useState<
    CommitteeAssignmentAutoAssignCommittee[] | null
  >(null);
  const [autoAssignLoading, setAutoAssignLoading] = useState(false);

  // eligible topic list state
  const [eligibleTopicList, setEligibleTopicList] = useState<
    EligibleTopicSummary[]
  >([]);
  const [eligibleLoading, setEligibleLoading] = useState(false);
  const [eligibleMode, setEligibleMode] = useState<"assign" | "multi-select">(
    "assign"
  );
  ...
  const cachedTags = useRef<
    { tagCode: string; tagName: string; description?: string | null }[] | null
  >(null);
  const cachedTagDictionary = useRef<
  ...
  const initializeWizard = useCallback(async () => {
    ...
    try {
      ...
      setWizardError(
        (error as Error).message || "Không thể khởi tạo dữ liệu tạo hội đồng"
      );
    } finally {
      setWizardInitializing(false);
    }
  ...
  const handlePhaseOneFieldChange = useCallback(
    (field: keyof PhaseOneFormState, value: string) => {
  ...
  const togglePhaseOneTag = useCallback((tagCode: string) => {
    setPhaseOneForm((prev) => {
      ...
      const tagCodes = exists
        ? prev.tagCodes.filter((code) => code !== tagCode)
  ...
  const fetchTopicsForSession = useCallback(async () => {
    ...
    try {
      ...
      const payload = response.data as ApiResponse<RawTopicItem[]>;
      const rawList = Array.isArray(payload?.data)
        ? (payload.data as RawTopicItem[])
        : Array.isArray(payload)
        ? (payload as RawTopicItem[])
        : Array.isArray(payload?.items)
        ? (payload.items as RawTopicItem[])
        : [];
      ...
      const eligible = mappedAll.filter(
        (t) => (t.status ?? "").trim() === "Đủ điều kiện bảo vệ"
      );
      ...
      if (topicCodes.length > 0) {
        try {
          ...
          const tagPayload = tagsResp.data as ApiResponse<TagItem[]>;
          const tagList = Array.isArray(tagPayload?.data)
            ? (tagPayload.data as TagItem[])
            : Array.isArray(tagPayload)
            ? (tagPayload as TagItem[])
            : [];
      ...
      const supervisorCodes = Array.from(
        new Set(
          eligible
            .map((t) => t.supervisorLecturerCode)
      ...
      if (supervisorCodes.length > 0) {
        try {
          ...
          const lecPayload = lecResp.data as ApiResponse<RawLecturerItem[]>;
          const lecRaw = Array.isArray(lecPayload?.data)
            ? (lecPayload.data as RawLecturerItem[])
            : Array.isArray(lecPayload)
            ? (lecPayload as RawLecturerItem[])
            : [];
          ...
          eligible.forEach((t) => {
            if (
              (!t.supervisorName || t.supervisorName === "—") &&
      ...
      addToast(
        (error as Error).message || "Không thể tải danh sách đề tài",
        "error"
      );
      setAvailableTopics([]);
    } finally {
  ...
  const fetchLecturersForCommittee = useCallback(async () => {
    ...
    try {
      ...
      const payload = response.data as ApiResponse<RawLecturerItem[]>;
      const rawList = Array.isArray(payload?.data)
        ? (payload.data as RawLecturerItem[])
        : Array.isArray(payload)
        ? (payload as RawLecturerItem[])
        : Array.isArray(payload?.items)
        ? (payload.items as RawLecturerItem[])
        : [];
      ...
      addToast(
        (error as Error).message || "Không thể tải danh sách giảng viên",
        "error"
      );
      setLecturerOptions([]);
    } finally {
  ...
  const lecturerDictionary = useMemo(() => {
    ...
    lecturerOptions.forEach((lecturer) =>
      map.set(lecturer.lecturerProfileId, lecturer)
    );
    return map;
  }, [lecturerOptions]);
  ...
  const handleAssignTopicToSession = useCallback(
    (topic: TopicTableItem, session: SessionId) => {
      if (!phaseOneForm.defenseDate) {
        addToast(
          "Vui lòng chọn ngày bảo vệ ở Bước 1 trước khi phân phiên.",
          "warning"
        );
        return;
      }
      ...
      if (
        currentSlots.some((slot) => slot.topic.topicCode === topic.topicCode)
      ) {
        return;
      }

      const otherFiltered = otherSlots.filter(
    ...
    [
      ...
      assignedTopics,
      phaseOneForm.defenseDate,
      rebuildSessionAssignments,
    ]
  );

  const handleRemoveTopicFromSession = useCallback(
    (topicCode: string, session: SessionId) => {
      const filtered = assignedTopics[session].filter(
        (slot) => slot.topic.topicCode !== topicCode
      );
  ...
  const handlePhaseOneConfirm = useCallback(async () => {
    ...
    try {
      ...
      const committeeCode =
        (wizardInit?.nextCode && String(wizardInit.nextCode).trim()) ||
        `COM${dateStr}001`;
      ...
      let createResponse;
      try {
        createResponse = await committeeAssignmentApi.createCommittee(
          createPayload
        );
      } catch (error) {
  ...
  const handleTopicsConfirm = useCallback(async () => {
    ...
    const items = rawItems.map((slot) => {
      const scheduledAt =
        slot.scheduledAt ??
        combineDateAndTime(phaseOneForm.defenseDate, slot.timeLabel);
    ...
    const scheduledTimes = items
      .map((i) => i.scheduledAt)
      .filter(Boolean) as string[];
    const topScheduledAt =
      scheduledTimes.length > 0 ? scheduledTimes.sort()[0] : null;
    ...
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        if (listResponse?.success && listResponse.data) {
          ...
          const nextSession = computeNextSessionCandidate(
          ...
          if (nextSession?.defenseDate) {
            try {
              const detail = await committeeAssignmentApi.getCommitteeDetail(
                nextSession.committeeCode,
                { signal }
              );
              if (!signal?.aborted && detail?.success && detail.data) {
                const soonest = extractSoonestAssignment(
                  detail.data.assignments
                );
                const baseTopicCount =
                  detail.data.assignments?.length ??
                  nextSession?.topicCount ??
                  0;
                ...
                nextSessionEnriched = {
                  ...
                  defenseDate:
                    soonest?.scheduledAt ??
                    detail.data.defenseDate ??
                    nextSession?.defenseDate ??
                    null,
                  room:
                    soonest?.room ??
                    detail.data.room ??
  ...
  useEffect(() => {
    ...
    fetchLecturersForCommittee,
    fetchTopicsForSession,
    wizardOpen,
    wizardStep,
  ]);
  ...
  const resolveWizardTagLabel = useCallback(
    (tagCode: string) => {
      const entry = wizardTagNameLookup[tagCode] ?? tagDictionary[tagCode];
      // Prefer human-friendly tagName, then description, then code
      return (
        (entry &&
          ((entry as TagDictionaryEntry).name ||
            (entry as TagDictionaryEntry).tagName)) ||
        (entry && (entry as TagDictionaryEntry).description) ||
        tagCode
      );
    },
    [tagDictionary, wizardTagNameLookup]
  ...
  const openEligibleModal = useCallback(
    async (
      config: {
        ...
        onConfirm?: (topics: EligibleTopicSummary[]) => void;
      } = {}
    ) => {
  ...
  const handleToggleEligibleTopic = useCallback(
    (topicCode: string, checked: boolean) => {
  ...
  const handleConfirmEligibleTopics = useCallback(() => {
    ...
    const selectedTopics = eligibleTopicList.filter((topic) =>
      selectedSet.has(topic.topicCode)
    );
    ...
    <div className="min-h-screen bg-[#F5F7FB] py-10">
                  ...
                  >
                    <div className="space-y-5">
                      <div className="rounded-2xl border border-[#E5ECFB] bg-[#F8FAFF] p-5">
                        ...
                        <p className="mt-2 text-sm text-[#4A5775]">
                          Bạn có thể thay đổi tên hội đồng bên dưới, mã sẽ tự
                          động đồng bộ khi tạo.
                        </p>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-2xl border border-[#E5ECFB] bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {wizardTagOptions.map((tagCode) => {
                            const selected =
                              phaseOneForm.tagCodes.includes(tagCode);
                      ...
                      <div className="rounded-2xl border border-[#E5ECFB] bg-[#F8FAFF] p-5 text-sm text-[#4A5775]">
                        <p>
                          Hệ thống sẽ gợi ý giảng viên và đề tài phù hợp dựa
                          trên tập tag bạn chọn. Bạn có thể quay lại bước này
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          <p className="text-sm font-semibold text-[#1F3C88]">
                            Chọn đề tài cho phiên bảo vệ
                          </p>
                          <p className="text-xs text-[#4A5775]">
                            Mỗi phiên tối đa 4 đề tài, tổng {DAILY_TOPIC_LIMIT}{" "}
                            đề tài trong cùng ngày.
                          </p>
                        </div>
                            ...
                            <tbody>
                              {filteredTopics.map((topic) => {
                                const inMorning = assignedTopics[1].some(
                                  (slot) =>
                                    slot.topic.topicCode === topic.topicCode
                                );
                                ...
                                const morningDisabled =
                                  !inMorning &&
                                  (assignedTopics[1].length >=
                                    SESSION_TOPIC_LIMIT ||
                                    (totalAssignedTopics >= DAILY_TOPIC_LIMIT &&
                                      !inAfternoon));
                                const afternoonDisabled =
                                  !inAfternoon &&
                                  (assignedTopics[2].length >=
                                    SESSION_TOPIC_LIMIT ||
                                    (totalAssignedTopics >= DAILY_TOPIC_LIMIT &&
                                      !inMorning));
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3 text-[#0F1C3F]">
                                      {topic.title}
                                    </td>
                                    <td className="px-4 py-3 text-[#4A5775]">
                                      {topic.studentName ?? "—"}
                                    ...
                                    <td className="px-4 py-3">
                                      <div className="flex flex-col gap-2 text-xs font-semibold sm:flex-row sm:justify-center">
                                        <button
                                          ...
                                          onClick={() =>
                                            handleAssignTopicToSession(topic, 1)
                                          }
                                        ...
                                        <button
                                          ...
                                          onClick={() =>
                                            handleAssignTopicToSession(topic, 2)
                                          }
                    ...
                    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                      <div className="flex flex-col gap-4 rounded-2xl border border-[#E5ECFB] bg-white p-5 shadow-sm">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-semibold text-[#1F3C88]">
                            Phân vai trò giảng viên
                          </span>
                          {lecturersLoading ? (
                            <span className="flex items-center gap-2 text-xs text-[#4A5775]">
                              <Loader2 className="h-4 w-4 animate-spin text-[#1F3C88]" />{" "}
                              Đang tải
                            </span>
                          ) : (
                            <span className="text-xs text-[#4A5775]">
                              {lecturerOptions.length.toLocaleString("vi-VN")}{" "}
                              lựa chọn
                            </span>
                          )}
                        </div>
                        <div className="space-y-4">
                          {ROLE_CONFIG.map((role) => {
                            ...
                            const options =
                              lecturerOptionsByRole[role.id] ?? [];
                            const selectedLecturer = selectedId
                              ? lecturerDictionary.get(selectedId)
                              : null;
                            return (
                              <div
                                key={role.id}
                              ...
                              >
                                <div className="flex items-center justify-between gap-3">
                                  <div>
                                    ...
                                    <p className="text-xs text-[#4A5775]">
                                      {role.requiresPhd
                                        ? "Yêu cầu học vị Tiến sĩ"
                                        : "Không trùng lặp với vai trò khác"}
                                    </p>
                                  </div>
                                  {selectedLecturer && (
                                ...
                                {options.length === 0 && !lecturersLoading && (
                                  <p className="mt-2 text-xs text-red-600">
                                    Không còn giảng viên phù hợp cho vai trò
                                    này.
                                  </p>
                                )}
                              </div>
                      ...
                      <div className="space-y-4">
                        <div className="rounded-2xl border border-[#E5ECFB] bg-white p-5 shadow-sm">
                          ...
                          <p className="text-xs text-[#4A5775]">
                            Kiểm tra giảng viên hướng dẫn để tránh xung đột.
                          </p>
                            ...
                            {[1, 2].map((sessionId) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    <span className="text-sm font-semibold text-[#1F3C88]">
                                      {SESSION_LABEL[typedSession]}
                                    </span>
                                    <span className="text-xs text-[#4A5775]">
                                      {slots.length}/{SESSION_TOPIC_LIMIT} đề
                                      tài
                                    </span>
                                  </div>
                                  {slots.length === 0 ? (
                        ...
                        <div className="rounded-2xl border border-[#FFE5D0] bg-[#FFF6EE] p-5 text-xs text-[#8B5E34]">
                          <p>
                            Không thể phân công giảng viên đã hướng dẫn đề tài
                            trong danh sách trên. Hệ thống sẽ thông báo nếu
                            trùng.
                          </p>
                          {supervisorCodesInAssignments.size > 0 && (
                            <p className="mt-2">
                              Mã giảng viên hướng dẫn hiện có:{" "}
                              {[...supervisorCodesInAssignments].join(", ")}.
                            </p>
                          )}
                        </div>
                      </div>
      ...
      {modals.eligibleTopics && (
        <EligibleTopicModal
          ...
          onToggleTopic={
            eligibleMode === "multi-select"
              ? handleToggleEligibleTopic
              : undefined
          }
          onConfirmSelection={
            eligibleMode === "multi-select"
              ? handleConfirmEligibleTopics
              : undefined
          }
          selectedTopicCodes={
            eligibleMode === "multi-select" ? eligibleSelectedCodes : []
          }
        />
      )}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setDeleting(true);
                  try {
                    const res = await committeeAssignmentApi.deleteCommittee(
                      deleteTarget,
                      true
                    );
                    if (res?.success) {
                    ...
                    addToast(
                      (err as Error)?.message ?? "Xóa thất bại",
                      "error"
                    );
                  } finally {
                    setDeleting(false);
